...
#MODEL
#------------------------
def extract_submatrices(WB):  # WB for weight and bias
    submatrices = []
    K = 0
    for i in range(0,len(layers)-1):
        result = []
        # FROM RELEVANT SUB MATRIX FOR LAYER-N
        Nrow = layers[i]
        Ncol = layers[i+1]
        w = np.array(WB[K:K+Nrow*Ncol].reshape(Nrow, Ncol))
        K = K + Nrow * Ncol
        Nrow = 1
        b = np.array(WB[K:K+Nrow*Ncol])
        K = K + Nrow * Ncol
        result.append(w)
        result.append(b)
...
def model(x,p):
    ...
    if(model_type=="linear" or model_type=="logistic" ):   
        ...
        for i in range(0,len(submatrices)):
            product = np.matmul(x,submatrices[i][0]) + submatrices[i][1]
            out = S(product)

        # for i in range(x.shape[0]):
        #     product = x[i]
        #     for j in range(len(layers) - 1):
        #         product = np.matmul(product,submatrices[j][0]) + submatrices[j][1] 
        #         product = np.tanh(product)
...
if model_type == 'ANN':
    ...
    po = np.random.uniform(0,1.,size = NFIT)
else: 
    po=np.random.uniform(0,1.,size=NFIT)
